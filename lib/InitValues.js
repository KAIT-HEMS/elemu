...
class InitValues {
    ...
    * ---------------------------------------------------------------- */
    init() {
        ...
        const json_path = mPath.resolve(__dirname, '../conf/initValues.json');
        const data = require(json_path);

        // common の変換
        const common = data['common'];
        if (common && typeof (common) === 'object') {
            const vals = this._getInitValues(common);
            if (vals) {
                this._common = vals;
            }
        }
        ...
        if (dev_list && Array.isArray(dev_list) && dev_list.length > 0) {
            ...
            dev_list.forEach((dev) => {
                ...
                const vals = this._getInitValues(dev);
                if (vals) {
                    devs[eoj] = vals;
                }
            });
    ...
    get(eoj) {
        ...
        if (this._caches[eoj]) {
            ...
            Object.keys(this._common).forEach((epc) => {
                if (!vals[epc]) {
                    const vlist = JSON.parse(JSON.stringify(this._common[epc]));
                    vals[epc] = vlist;
                }
            });
