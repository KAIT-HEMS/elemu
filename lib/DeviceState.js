    ...
    setEpcValues(props, is_admin, set_delay_msec) {
        return new Promise((resolve, reject) => {
            ...
            for (let epc of epc_list) {
                ...
                // EDT のチェック
                const prop = this._desc['elProperties'][epc];
                if (!prop || !prop['data']) {
            ...
            const cvals = {};
            for (let [epc, val] of Object.entries(uvals)) {
                if (this._states[epc] !== val) {
                    cvals[epc] = val;
                }
            }
            ...
            // 個別処理: 計測履歴系 EPC
            const energy_historical_vals = this._setElectricEnergyHistoricalData(uvals, cvals);

            // 戻り値生成
            const rvals = {};
            for (let [epc, val] of Object.entries(vals)) {
                if (uvals[epc]) {
                    rvals[epc] = null;
                } else {
                    rvals[epc] = val;
            ...
            // ファイルへ書き込み
            const svals = JSON.parse(JSON.stringify(cvals));
    ...
    _isValidEdt(val, pdata, is_admin) {
        ...
        if (type === 'number') {
            ...
            for (let i = 0, len = values.length; i < len; i++) {
                let edt = values[i]['edt'].replace(/^0x/, '');
                if (edt === val) {
                    valid = true;
                    break;
                }
